...
class PrecisionDockingNode(Node):
    ...
    def __init__(self, target_bed_id=1):
        ...
        self.docking_start_yaw = 0.0  # 👈 이 줄을 추가해서 변수를 미리 만들어둡니다.
        # ── 추가 직진 (State 5) ──────────────────────────────
        self.start_x          = 0.0
        self.start_y          = 0.0
        self.extra_fwd_target = 0.0
        # ── [추가] 좌표 기반 이동을 위한 전역 변수들 ──
        self.latest_camera_dist = 0.0
        self.goal_x = 0.0
        self.goal_y = 0.0
        self.pillar_x = 0.0
        self.pillar_y = 0.0
...
# ── State 5 : 좌표 기반 정밀 이동 (조향 강화 버전) ──
            ...
            self.get_logger().info(f"🚀 [좌표 이동] 남은거리: {dist_to_goal:.2f}m | 오차: {math.degrees(heading_error):.1f}°")

            if dist_to_goal > 0.08:  
                # 💡 [핵심 수정] 오차가 클수록 직진 속도를 부드럽게 줄이는 마법의 공식!
                # (오차가 0도면 100% 속도, 오차가 45도면 0% 속도)
                speed_factor = max(0.0, 1.0 - (abs(heading_error) / math.radians(45)))
                
                # 거리에 따른 감속 + 각도 오차에 따른 감속 동시 적용
                base_speed = min(0.15, dist_to_goal * 0.5)
                twist.linear.x = base_speed * speed_factor 

                # 조향은 그대로 강력하게!
                cmd_w = heading_error * 2.0
                min_w = 0.15
                if cmd_w > 0:
                    twist.angular.z = max(min_w, min(0.5, cmd_w))
                else:
                ...
                self.target_yaw = self.normalize_angle(self.bed_yaw + (turn_rad if self.park_side == 1 else -turn_rad))
                self.change_state(1, "🔄 [State 1] 목표지점 도착! 180도 회전 시작")
        # ── State 6 : 진짜 주차 완료 ──────────
        ...
        if self.state == 0:
            ...
            if contours:
                ...
                if cv2.contourArea(c) > 300:
                    ...
                    self.latest_camera_dist = camera_dist_m
                    ...
                    if camera_dist_m > target_dist_m:
                    ...
                    else: # 목표 거리(target_dist_m)에 도달했을 때
                        twist.linear.x  = 0.0
                        twist.angular.z = 0.0
                        ...
                        # 🎯 3. [교체 포인트] 목표 좌표(Waypoint) 계산
                        # offset_x: 기둥에서 침대 발치 쪽으로 1.2m 앞
                        # offset_y: 침대 방향에 따라 기둥 옆으로 0.6m 비켜나기
                        if self.current_bed_id == 4: # 🟥 빨간색 침대 (왼쪽)
                            offset_x = -0.5  # 사용자님이 찾으신 완벽한 앞뒤 거리
                            offset_y = -0.5  # 방금 수정한 스무스한 옆 간격
                            
                        elif self.current_bed_id == 1: # 🟦 파란색 침대 (오른쪽)
                            # 예전에 잘 되던 "단순 80cm 직진"을 좌표로 그대로 구현!
                            offset_x = -0.5  # 앞으로 80cm
                            offset_y = 0.1 # 옆으로는 꺾지 않음 (대각선 주행 X)
                            
                        else: # 노랑, 보라 침대 (필요시 추가 튜닝)
                            offset_x = 0.0
                            offset_y = 0.0
                        
                        self.goal_x = self.pillar_x + (offset_x * math.cos(self.bed_yaw) - offset_y * math.sin(self.bed_yaw))
                        self.goal_y = self.pillar_y + (offset_x * math.sin(self.bed_yaw) + offset_y * math.cos(self.bed_yaw))

                        self.get_logger().info(f"📍 [목표 설정] 좌표: ({self.goal_x:.2f}, {self.goal_y:.2f})")
                        self.change_state(5, "🚀 [State 5] 목표 좌표로 정밀 이동 시작!")
            self.publish_safe_twist(twist)
