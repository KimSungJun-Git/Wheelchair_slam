...
class DockingControlNode(Node):
    def __init__(self):
        ...
        # ROS2 파라미터 (명령어로 실시간 튜닝 가능)
        self.declare_parameter('tube_radius', 0.60)
        self.declare_parameter('drive_speed', 0.15)
        self.declare_parameter('arrive_dist', 0.40)
        self.declare_parameter('lidar_stop_dist', 0.25)
        ...
        self.get_logger().info('DockingControlNode 준비 (TF2 + 부드러운 스무딩 필터 적용)')
    ...
    def _target_cb(self, msg: PointStamped):
        ...
        if odom_msg:
            new_x, new_y = odom_msg.point.x, odom_msg.point.y
            
            if not self.target_locked:
                # 처음 발견하면 해당 좌표로 락온(Lock-on)
                self.goal_x, self.goal_y = new_x, new_y
                self.target_locked = True
            else:
                # ⭐️ EMA 필터: 이전 값을 90% 믿고, 새 값은 10%만 반영하여 떨림 방지
                alpha = 0.1
                self.goal_x = (1 - alpha) * self.goal_x + alpha * new_x
                self.goal_y = (1 - alpha) * self.goal_y + alpha * new_y
    ...
    def _tag_cb(self, msg: PointStamped):
        ...
        if odom_msg:
            new_x, new_y = odom_msg.point.x, odom_msg.point.y
            if not self.target_locked:
                self.obs_x, self.obs_y = new_x, new_y
            else:
                # 기둥(마커) 위치도 똑같이 스무딩 처리
                alpha = 0.1
                self.obs_x = (1 - alpha) * self.obs_x + alpha * new_x
                self.obs_y = (1 - alpha) * self.obs_y + alpha * new_y
    ...
    def _loop(self):
        ...
        if self.state == State.SEARCHING:
            if not self.target_locked:
                ...
                self.get_logger().info('🗺️ 목표물 포착 완료! 도킹 시작')
