...
class TagPoseNode(Node):
    ...
    def _det_cb(self, msg: AprilTagDetectionArray):
        ...
        for det in msg.detections:
            # ⭐️ 인식 버그 방지 로직
            try:
                current_id = int(det.id[0])
            except TypeError:
                current_id = int(det.id)
