...
class PrecisionDockingNode(Node):
    ...
    def __init__(self, target_bed_id=1):
        ...
        cv2.setTrackbarPos("Bed_Side(0:L,1:R)", "Tuning Panel", 1)
        cv2.setTrackbarPos("Target_Dist(cm)",   "Tuning Panel", 150)
        cv2.setTrackbarPos("Extra_Fwd_L(cm)",   "Tuning Panel", 0)
    ...
    def scan_callback(self, msg):
        ...
        # ── State 2 : 라이다 벽면 법선 탐색 정렬 ────────
            ...
            idx_error = min_idx - 180 ######### 수정
            angle_error_rad = math.radians(idx_error)

            sym_offset = 20
            left_dists = [self.get_valid_dist(msg.ranges, i) 
                          for i in range(max(arc_start, min_idx - sym_offset), min_idx)]
            right_dists = [self.get_valid_dist(msg.ranges, i) 
                           for i in range(min_idx + 1, min(arc_end, min_idx + sym_offset + 1))]

            left_valid = [d for d in left_dists if d < 2.0]
            right_valid = [d for d in right_dists if d < 2.0]

            left_val = float(np.median(left_valid)) if left_valid else min_dist
            right_val = float(np.median(right_valid)) if right_valid else min_dist
            ...
            sym_ok = abs(sym_error) < 0.025

            if idx_ok and sym_ok:
                
                # ⭐️ [분기 처리] 중앙 침대(빨간색: 4번)는 이미 평행하므로 회전(State 4) 생략!
                if self.current_bed_id == 4:
                    self.start_x = self.robot_x
                    self.start_y = self.robot_y
                    self.target_yaw = self.current_yaw # 현재 각도 그대로 유지!
                    self.change_state(6, "➡️ [State 6] 빨간 침대 - 회전 생략하고 최종 나란히 이동!")
                
                # 파란색(1번) 등 벽이 있는 침대는 원래대로 90도 회전
                else:
